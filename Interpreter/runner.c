...
bool handle_op(dvalue_t* tok, darray_t* stk, darray_t* defs) {
  char* op = tok->d.s;
  if (false) {}
  ...
  else if (strcmp(op, "do") == 0)       { if(!op_do(stk)) return false; }
  else if (strcmp(op, "loop") == 0)     { if(!op_loop(stk)) return false; }
  else if (strcmp(op, "times") == 0)    { if(!op_times(stk)) return false; }
  else if (strcmp(op, "fi") == 0)       { if(!op_fi(stk)) return false; }
